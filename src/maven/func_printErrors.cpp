...
void printErrors(MavenCompiler* c) {
	for(int i = 0; i < c->errors.length(); ++i) {
		...
		int grouped = 0;
		while(i + grouped < c->errors.length() && c->errors[i].file == c->errors[i + grouped].file && c->errors[i].line == c->errors[i + grouped].line)
			++grouped;
		...
		if(c->errors[i].file.length() > 0)
			msg = string(" (") + c->errors[i].file + ", line " + intToString(c->errors[i].line) + "): ";
		else msg = ": ";
		pad = msg.length();
		cerr << msg;
		...
		for(int j = 0; j < grouped; ++j) {
			...
			fprintf(stderr, c->errors[i + j].message.c_str(), c->errors[i + j].arg1.c_str(), c->errors[i + j].arg2.c_str());
			cerr << endl;
		}
