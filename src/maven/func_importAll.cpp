...
bool importAll(MavenCompiler* c, string import, bool compiling) {
	...
	if(dp != NULL) {
		while(ep = readdir(dp)) {
			...
			string filepath = combinePaths(c->currentDirectory, c->iniFile.getKey("directories.import")) + parts[0] + "/" + ep->d_name;
			if(alreadyImported(c, filepath)) continue;
			...
			--c->lineNumber;
			if(compiling) compileFile(c, filepath, MAVEN_ONLY_COMPILE);
