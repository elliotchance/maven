...
string resolveVariable(MavenCompiler* c, string entity, MavenVariable& v, int& namespaceID, int& objectID, bool& isLocal, bool forFunction) {
	...
	if(elementID == -2) {
		...
		v.type = c->namespaces[namespaceID].name + "." + c->namespaces[namespaceID].enums[enumID].name;
		return intToString(c->namespaces[namespaceID].enums[enumID].items[elementID].value);
	}
	...
	if(namespaceID >= 0 && objectID >= 0) {
		for(int i = 0; i < c->namespaces[namespaceID].objects[objectID].variables.length(); ++i) {
			if(c->namespaces[namespaceID].objects[objectID].variables[i].name == entity) {
				v = c->namespaces[namespaceID].objects[objectID].variables[i];
				string r = entity;
	...
	for(int i = 0; i < c->namespaces.length(); ++i) {
		for(int j = 0; j < c->namespaces[i].objects[0].variables.length(); ++j) {
			if(c->namespaces[i].objects[0].variables[j].name == entity) {
				ambig.push_back(i);
				v = c->namespaces[i].objects[0].variables[j];
			}
		}
	}
	...
	if(ambig.size() == 1) {
		...
		return c->namespaces[namespaceID].name + "::nil$static::" + entity;
	...
	if(namespaceID >= 0) {
		v.name = c->namespaces[namespaceID].name;
		v.type = "<Namespace: " + v.name + ">";
		return c->namespaces[namespaceID].name;
	}
	...
	if(enumID >= 0) {
		v.name = c->namespaces[namespaceID].name + "." +
				 c->namespaces[namespaceID].enums[enumID].name;
		v.type = "<Enum: " + v.name + ">";
		return c->namespaces[namespaceID].name + "::" +
			   c->namespaces[namespaceID].enums[enumID].name;
	}
	...
	if(namespaceID >= 0 && objectID >= 0) {
		for(int i = 0; i < c->namespaces[namespaceID].objects[objectID].functions.length(); ++i) {
			if(c->namespaces[namespaceID].objects[objectID].functions[i].name == entity) {
